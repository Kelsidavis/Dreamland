...
def export_markdown(conv: Conversation, include_metadata: bool = False) -> str:
    ...
    lines.append("---")
    lines.append(
        "*Exported from [Dreamland](https://github.com/Kelsidavis/Dreamland) "
        "— It doesn't exist.*"
    )
...
def export_text(conv: Conversation) -> str:
    ...
    for msg in conv.messages:
        match msg.role:
            ...
            case Role.ASSISTANT:
                lines.append(f"[dreamland] {msg.content}")
            case Role.TOOL:
                content = msg.content
                if len(content) > 300:
                    content = content[:300] + "..."
...
def export_html(conv: Conversation, include_metadata: bool = True) -> str:
    ...
    for msg in conv.messages:
        ...
        label = {"user": "You", "assistant": "Dreamland", "tool": "Tool", "system": "System"}
    ...
    parts.append(
        'Exported from <a href="https://github.com/Kelsidavis/Dreamland">'
        "Dreamland</a> &mdash; It doesn't exist."
    )
    parts.append("</div>")
