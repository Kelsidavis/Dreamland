...
Usage:
    ...
    # Then run Dreamland:
    dreamland chat --backend llama
    dreamland chat --backend llama --llama-url http://gpu-box:8080
"""
...
from dreamland.agent.runtime import format_tool_feedback, tool_result_is_error
from dreamland.agent.tool_parser import ToolCall, parse_tool_calls
from dreamland.agent.tools_payload import tools_as_openai_functions
from dreamland.config import DreamlandConfig
from dreamland.skills.registry import SkillRegistry

log = logging.getLogger("dreamland.agent.llama")
...
class LlamaRuntime:
    ...
    Usage:
        dreamland chat --backend llama
        dreamland chat --backend llama --llama-url http://gpu-box:8080
        dreamland worker --master ws://... --backend llama --llama-url http://gpu-box:8080
    """
    ...
    async def load_model(self) -> None:
        ...
        from dreamland.agent.discovery import ManagedLlamaServer, detect_system
        ...
        if not model_path:
            ...
            if not best:
                raise RuntimeError(
                    ...
                    "~/.dreamland/models/, or use --llama-model to specify one."
                )
    ...
    def build_inference_request(self, conversation: Conversation) -> dict[str, Any]:
        ...
        See dreamland.nodes.roles.task_needs_tools / task_wants_thinking.
        """
