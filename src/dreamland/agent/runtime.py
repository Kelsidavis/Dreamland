...
from dreamland.agent.instance_lock import acquire_runtime_lock
...
log = logging.getLogger("dreamland.agent")
...
# src/dreamland/skills/ return ``f"Error: {e}"`` or ``f"Error reading X: {e}"``
...
class AgentRuntime:
    ...
    def _make_turboquant_cache(self) -> list | None:
        ...
        if not self.config.model.turboquant:
            return None
        from dreamland.agent.turboquant import make_turboquant_cache
    ...
    def _run_capture_hooks(self, query: str | None) -> None:
        """Thin shim around dreamland.agent.capture.run_capture_hooks.
    ...
    def _build_system_content(
        ...
        # otherwise discover from local .dreamland.md files
    ...
    def _detect_native_tools_support(self) -> bool:
        ...
        try:
            ...
            return False
        return "__dreamland_probe__" in (with_tools or "")
