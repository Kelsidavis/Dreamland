"""Project context loader — discovers and loads .dreamland.md files.

Walks up from the current working directory looking for .dreamland.md,
then loads all of them (child first, parent last) to build a
project context block for the system prompt.

Also supports .dreamland/ directories containing multiple context files
for larger projects.

Discovery order (all loaded, child wins on conflict):
  1. .dreamland.md in cwd
  2. .dreamland.md in parent directories (up to home or root)
  3. .dreamland/*.md files in the nearest .dreamland/ directory
"""
...
log = logging.getLogger("dreamland.agent.project")

CONTEXT_FILENAME = ".dreamland.md"
CONTEXT_DIR = ".dreamland"
...
def find_project_contexts(start_dir: Path | None = None) -> list[Path]:
    """Find .dreamland.md files walking up from start_dir.
    ...
    current = start
    while True:
        # Check for .dreamland.md file
        candidate = current / CONTEXT_FILENAME
        if candidate.is_file():
            found.append(candidate)

        # Check for .dreamland/ directory with markdown files
