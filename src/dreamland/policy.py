...
Modes (env ``DREAMLAND_TOOL_POLICY``):
...
* ``enforce`` — refuse any tool whose risk tier is in the blocked set
  (env ``DREAMLAND_BLOCKED_RISKS``, default ``exec,exfil,secret,persist,
  lateral``), unless the specific tool is named in the allowlist.

Overrides (comma-separated tool names):

* ``DREAMLAND_ALLOW_TOOLS`` — always allow these, even under enforce.
* ``DREAMLAND_DENY_TOOLS`` — always refuse these, in any mode.

Risk tiers come from ``dreamland.audit.risk_tag`` so the gating model and the
...
from dreamland.audit import risk_tag

log = logging.getLogger("dreamland.policy")
...
class ToolPolicy:
    ...
    def from_env(cls, config: object | None = None) -> ToolPolicy:
        ...
        env_mode = os.environ.get("DREAMLAND_TOOL_POLICY")
        env_blocked = os.environ.get("DREAMLAND_BLOCKED_RISKS")
        env_allow = os.environ.get("DREAMLAND_ALLOW_TOOLS")
        env_deny = os.environ.get("DREAMLAND_DENY_TOOLS")
    ...
    def evaluate(self, tool_name: str) -> str | None:
        ...
        if tier in self.blocked_risks:
            return (
                ...
                "by the active tool policy. Add it to DREAMLAND_ALLOW_TOOLS to "
                "permit, or relax DREAMLAND_BLOCKED_RISKS."
            )
        return None
...
def get_policy() -> ToolPolicy:
    ...
    menu → ``~/.dreamland/config.toml``), with environment variables taking
    ...
    if _policy is None:
        ...
        try:
            ...
            security = DreamlandConfig.load().security
