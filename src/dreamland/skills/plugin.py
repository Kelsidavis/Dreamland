...
A plugin is a directory with a dreamland-plugin.toml manifest:

    my-plugin/
        dreamland-plugin.toml
        skill.py            (or __init__.py)

Manifest format (dreamland-plugin.toml):
    [plugin]
    name = "weather"
    version = "1.0.0"
...
log = logging.getLogger("dreamland.skills.plugin")

PLUGINS_DIR = DREAMLAND_HOME / "plugins"
...
class PluginManifest:
    ...
    def from_toml(cls, path: Path) -> PluginManifest | None:
        """Parse a dreamland-plugin.toml file."""
        try:
            import toml
            ...
            return cls(
                ...
                min_dreamland=plugin.get("min_dreamland", ""),
...
def discover_plugins(dirs: list[Path] | None = None) -> list[PluginManifest]:
    ...
    for d in search_dirs:
        ...
        for entry in sorted(d.iterdir()):
            if entry.is_dir():
                manifest_path = entry / "dreamland-plugin.toml"
...
def validate_plugin(path: Path) -> list[str]:
    ...
    manifest_path = path / "dreamland-plugin.toml"
    if not manifest_path.exists():
        issues.append("Missing dreamland-plugin.toml")
        return issues

    manifest = PluginManifest.from_toml(manifest_path)
    if not manifest:
        issues.append("Failed to parse dreamland-plugin.toml")
        return issues
...
name = "{name}"
version = "0.1.0"
description = "A custom Dreamland plugin"
author = ""
license = "MIT"
min_dreamland = "0.5.0"
tags = []
...
'''
    ...
    (target / "dreamland-plugin.toml").write_text(manifest, encoding="utf-8")
