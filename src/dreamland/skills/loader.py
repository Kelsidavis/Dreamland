...
Skill directories can contain:
  ...
  3. Manifest skills:     my_skill/    (package with dreamland-skill.toml metadata)
...
from dreamland.skills.base import Skill
from dreamland.skills.registry import SkillRegistry

log = logging.getLogger("dreamland.skills.loader")
...
class SkillLoader:
    ...
    def _load_module_file(self, path: Path) -> list[Skill]:
        ...
        module_name = f"dreamland_skill_{path.stem}"
    ...
    def _load_package(self, path: Path) -> list[Skill]:
        ...
        module_name = f"dreamland_skill_{path.name}"
