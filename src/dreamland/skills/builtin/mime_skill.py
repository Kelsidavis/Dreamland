...
class MimeSkill(Skill):
    ...
    def description(self) -> str:
        return "Identify file MIME types by extension or path"

    def tools(self) -> list[ToolDefinition]:
        return [
            ToolDefinition(
                name="mime_type",
                description="Get MIME type for a file extension or path",
                parameters={
                    "type": "object",
                    "properties": {
                        "path": {
                            "type": "string",
                            "description": "File path or extension (e.g., .json, photo.png)",
                        }
                    },
                    "required": ["path"],
                },
    ...
    async def execute(self, tool_name: str, arguments: dict[str, Any]) -> Any:
        if tool_name == "mime_type":
            ...
            if not exts:
                return f"No extensions for: {arguments['mime']}"
