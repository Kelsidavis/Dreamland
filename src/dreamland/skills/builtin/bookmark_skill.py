...
BM_FILE = DREAMLAND_HOME / "bookmarks.json"
...
class BookmarkSkill(Skill):
    ...
    def description(self) -> str:
        return "Save, search, and organize URL bookmarks"
    ...
    async def execute(self, tool_name: str, arguments: dict[str, Any]) -> Any:
        match tool_name:
            case "bookmark_add":
                return self._add(
                    arguments["url"], arguments.get("title", ""), arguments.get("tags", [])
                )
            case "bookmark_search":
                return self._search(arguments.get("query", ""), arguments.get("tag"))
            case "bookmark_list":
                return self._list(arguments.get("limit", 20))
    ...
    def _search(self, query: str, tag: str | None) -> str:
        ...
        for i, b in enumerate(bms):
            if tag and tag.lower() not in b.get("tags", []):
                continue
            if q and q not in b["url"].lower() and q not in b.get("title", "").lower():
                continue
            results.append((i, b))
        if not results:
