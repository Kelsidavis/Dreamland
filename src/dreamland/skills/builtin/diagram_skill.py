...
class DiagramSkill(Skill):
    ...
    def description(self) -> str:
        return "Generate ASCII diagrams — flowcharts, trees, sequence diagrams"

    def tools(self) -> list[ToolDefinition]:
        return [
            ToolDefinition(
                name="diagram_tree",
                description="Draw a tree/hierarchy diagram from nested data",
                parameters={
                    "type": "object",
                    "properties": {
                        ...
                        "children": {
                            "type": "string",
                            "description": "Children as indented text (2-space indent per level)",
                        },
                    },
            ...
            ToolDefinition(
                name="diagram_flow",
                description="Draw an ASCII flowchart from a list of steps",
                parameters={
                    "type": "object",
                    "properties": {
                        "steps": {
                            "type": "array",
                            "items": {"type": "string"},
                            "description": "Flowchart steps in order",
                        },
                        "direction": {
                            "type": "string",
            ...
            ToolDefinition(
                ...
                description="Draw an ASCII sequence diagram from actors and messages",
                parameters={
                    "type": "object",
                    "properties": {
                        "actors": {
                        ...
                        "messages": {
                            "type": "array",
                            "items": {"type": "string"},
                            "description": "Messages as 'from->to: text'",
                        },
                    },
    ...
    async def execute(self, tool_name: str, arguments: dict[str, Any]) -> Any:
        match tool_name:
            case "diagram_tree":
                return self._tree(arguments["root"], arguments["children"])
            case "diagram_flow":
                return self._flow(arguments["steps"], arguments.get("direction", "vertical"))
            case "diagram_sequence":
                return self._sequence(arguments["actors"], arguments["messages"])
            case _:
    ...
    def _tree(self, root: str, children_text: str) -> str:
        ...
        for i, child in enumerate(children):
            ...
            is_last = (i == len(children) - 1) or (
                i + 1 < len(children)
                and (len(children[i + 1]) - len(children[i + 1].lstrip())) // 2 <= indent
            )
            prefix = ""
            for d in range(indent):
                prefix += "│   " if d < indent - 1 else ("└── " if is_last else "├── ")
            if indent == 0:
                _is_last_root = (i == len(children) - 1) or (
                    i + 1 < len(children) and children[i + 1].startswith("  ")
                )
    ...
    def _flow(self, steps: list[str], direction: str) -> str:
        if direction == "horizontal":
            ...
            return " --> ".join(boxes)
        lines = []
        _max_len = max(len(s) for s in steps) + 4
