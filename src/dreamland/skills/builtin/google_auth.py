...
Stores credentials at ~/.dreamland/google_credentials.json.
...
log = logging.getLogger("dreamland.skills.google_auth")

DREAMLAND_HOME = Path.home() / ".dreamland"
CREDS_PATH = DREAMLAND_HOME / "google_credentials.json"
TOKEN_PATH = DREAMLAND_HOME / "google_token.json"

# OAuth client config — desktop app type (installed)
# Users should replace these with their own from Google Cloud Console,
# or set DREAMLAND_GOOGLE_CLIENT_ID / DREAMLAND_GOOGLE_CLIENT_SECRET env vars.
DEFAULT_CLIENT_ID = ""
...
def _load_client_config() -> dict[str, str]:
    ...
    client_id = os.environ.get("DREAMLAND_GOOGLE_CLIENT_ID", "")
    client_secret = os.environ.get("DREAMLAND_GOOGLE_CLIENT_SECRET", "")
...
def get_google_credentials() -> Any:
    ...
    if not config["client_id"]:
        raise RuntimeError(
            ...
            "1. Place your OAuth client JSON at ~/.dreamland/google_credentials.json\n"
            "2. Set DREAMLAND_GOOGLE_CLIENT_ID and DREAMLAND_GOOGLE_CLIENT_SECRET env vars\n"
