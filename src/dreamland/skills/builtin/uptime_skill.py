...
class UptimeSkill(Skill):
    ...
    def description(self) -> str:
        return "Check if URLs are up and track response times"
    ...
    def tools(self) -> list[ToolDefinition]:
        return [
            ...
            ToolDefinition(
                name="uptime_history",
                description="Show recent check history",
                parameters={
                    "type": "object",
                    "properties": {
    ...
    async def execute(self, tool_name: str, arguments: dict[str, Any]) -> Any:
        match tool_name:
            case "uptime_check":
                return await self._check(arguments["url"], arguments.get("expected_status", 200))
            case "uptime_batch":
                return await self._batch(arguments["urls"])
            case "uptime_history":
                return self._hist(arguments.get("limit", 20))
            case _:
    ...
    async def _check(self, url: str, expected: int) -> str:
        ...
        try:
            async with httpx.AsyncClient(timeout=10, follow_redirects=True) as c:
                resp = await c.get(url, headers={"User-Agent": "Dreamland-Uptime/1.0"})
    ...
    def _hist(self, limit: int) -> str:
        ...
        for e in _history[:limit]:
            ...
            lines.append(
                f"  [{icon}] {e['time']} {e['url'][:40]} {e['status']} {e['code']} {e['ms']}ms"
            )
        return "\n".join(lines)
