...
class KeychainSkill(Skill):
    ...
    def description(self) -> str:
        return "Securely store and retrieve secrets via macOS Keychain"

    def tools(self) -> list[ToolDefinition]:
        return [
            ToolDefinition(
                name="secret_set",
                description="Store a secret in the system keychain",
                parameters={
                    "type": "object",
                    "properties": {
            ...
            ToolDefinition(
                name="secret_list",
                description="List stored secret names (values hidden)",
    ...
    async def execute(self, tool_name: str, arguments: dict[str, Any]) -> Any:
        ...
        match tool_name:
            case "secret_set":
                return await self._set(arguments["name"], arguments["value"])
            case "secret_get":
                return await self._get(arguments["name"])
            case "secret_delete":
                return await self._delete(arguments["name"])
            case "secret_list":
    ...
    async def _get(self, name: str) -> str:
        code, out = await self._run(["find-generic-password", "-s", SERVICE, "-a", name, "-w"])
        if code != 0:
            return f"Not found: {name}"
        return out

    async def _delete(self, name: str) -> str:
        code, out = await self._run(["delete-generic-password", "-s", SERVICE, "-a", name])
        return f"Deleted: {name}" if code == 0 else f"Not found: {name}"

    async def _list(self) -> str:
        code, out = await self._run(["dump-keychain"])
        if code != 0:
            return "Cannot read keychain."
        import re

        names = set()
        for m in re.finditer(
            r'"svce"<blob>="dreamland-secrets".*?"acct"<blob>="([^"]+)"', out, re.DOTALL
        ):
            names.add(m.group(1))
        if not names:
        ...
        if not names:
            return "No dreamland secrets stored."
