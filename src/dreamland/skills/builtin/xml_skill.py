...
def _xml_to_dict(xml: str) -> dict | list | str:
    ...
    def parse_element(s):
        ...
        if not m:
            return s.strip()
        tag, _attrs, inner = m.group(1), m.group(2), m.group(3).strip()
...
class XmlSkill(Skill):
    ...
    def description(self) -> str:
        return "Parse XML to JSON and extract data"

    def tools(self) -> list[ToolDefinition]:
        return [
            ToolDefinition(
                name="xml_to_json",
                description="Convert XML text to JSON",
                parameters={
                    "type": "object",
            ...
            ToolDefinition(
                name="xml_extract",
                description="Extract values matching an XPath-like pattern from XML",
                parameters={
                    "type": "object",
                    "properties": {
                        "xml": {"type": "string"},
                        "tag": {"type": "string", "description": "Tag name to extract"},
                    },
                    "required": ["xml", "tag"],
                },
            ),
    ...
    async def execute(self, tool_name: str, arguments: dict[str, Any]) -> Any:
        if tool_name == "xml_to_json":
            ...
            if not matches:
                return f"No <{tag}> elements found."
