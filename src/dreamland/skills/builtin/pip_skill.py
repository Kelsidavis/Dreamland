...
class PipSkill(Skill):
    ...
    def description(self) -> str:
        return "Inspect Python dependencies, virtual environments, and project config"

    def tools(self) -> list[ToolDefinition]:
        return [
            ToolDefinition(
                name="pip_list",
                description="List installed Python packages (in current env)",
                parameters={
                    "type": "object",
                    "properties": {
            ...
            ToolDefinition(
                ...
                description="Parse and analyze a requirements.txt file",
                parameters={
                    "type": "object",
                    "properties": {
                        "path": {
                            "type": "string",
                            "description": "Path to requirements.txt (default: requirements.txt)",
                        },
                    },
                },
            ),
            ToolDefinition(
                name="pip_venv_info",
                description="Show info about the current Python/virtual environment",
    ...
    async def execute(self, tool_name: str, arguments: dict[str, Any]) -> Any:
        match tool_name:
            case "pip_list":
                return await self._list(arguments.get("filter"))
            case "pip_requirements":
                return self._requirements(arguments.get("path", "requirements.txt"))
            case "pip_venv_info":
    ...
    async def _list(self, name_filter: str | None) -> str:
        try:
            ...
            if name_filter:
                ...
                header = lines[:2]
                filtered = [line for line in lines[2:] if name_filter.lower() in line.lower()]
                return (
                    "\n".join(header + filtered)
                    if filtered
                    else f"No packages matching '{name_filter}'"
                )
            return output
