...
class ColorSkill(Skill):
    ...
    def description(self) -> str:
        return "Convert colors between hex/RGB/HSL, generate palettes"
    ...
    def tools(self) -> list[ToolDefinition]:
        return [
            ToolDefinition(
                ...
                parameters={
                    ...
                    "properties": {
                        "color": {
                            "type": "string",
                            "description": "Color value (e.g., #ff6600, rgb(255,102,0), red)",
                        },
                    },
                    "required": ["color"],
                },
            ...
            ToolDefinition(
                name="color_palette",
                description="Generate a color palette (complementary, analogous, triadic)",
                parameters={
                    "type": "object",
                    "properties": {
                        ...
                        "type": {
                            "type": "string",
                            "enum": ["complementary", "analogous", "triadic", "shades"],
            ...
            ToolDefinition(
                name="color_contrast",
                description="Check contrast ratio between two colors (WCAG accessibility)",
                parameters={
                    "type": "object",
                    "properties": {
    ...
    async def execute(self, tool_name: str, arguments: dict[str, Any]) -> Any:
        match tool_name:
            case "color_convert":
                return self._convert(arguments["color"])
            case "color_palette":
                return self._palette(arguments["base"], arguments.get("type", "complementary"))
            case "color_contrast":
                return self._contrast(arguments["color1"], arguments["color2"])
            case _:
    ...
    def _palette(self, base: str, ptype: str) -> str:
        ...
        if ptype == "complementary":
            colors.append(_hsl_to_hex((h + 180) % 360, s, lum))
        elif ptype == "analogous":
            colors.extend([
                _hsl_to_hex((h + 30) % 360, s, lum),
                _hsl_to_hex((h - 30) % 360, s, lum),
            ])
        elif ptype == "triadic":
            colors.extend([
                _hsl_to_hex((h + 120) % 360, s, lum),
                _hsl_to_hex((h + 240) % 360, s, lum),
            ])
        elif ptype == "shades":
            for lv in [max(lum - 20, 0), max(lum - 10, 0), min(lum + 10, 100), min(lum + 20, 100)]:
...
def _hsl_to_hex(h: int, s: int, lum: int) -> str:
    s2, l2 = s / 100, lum / 100
    c = (1 - abs(2 * l2 - 1)) * s2
    x = c * (1 - abs((h / 60) % 2 - 1))
    m = l2 - c / 2
