...
class DotenvSkill(Skill):
    ...
    def description(self) -> str:
        return "Read, validate, and compare .env files"

    def tools(self) -> list[ToolDefinition]:
        return [
            ToolDefinition(
                name="dotenv_read",
                description="Read and parse a .env file",
                parameters={
                    "type": "object",
                    "properties": {
                        "path": {
            ...
            ToolDefinition(
                ...
                description="Check a .env file against an .env.example template",
                parameters={
                    "type": "object",
                    "properties": {
                        "env_path": {
                            "type": "string",
                            "description": "Path to .env (default: .env)",
                        },
                        "template_path": {
                            "type": "string",
                            "description": "Path to template (default: .env.example)",
                        },
                    },
                    "required": [],
                },
    ...
    async def execute(self, tool_name: str, arguments: dict[str, Any]) -> Any:
        match tool_name:
            case "dotenv_read":
                return self._read(arguments.get("path", ".env"))
            case "dotenv_validate":
                return self._validate(
                    arguments.get("env_path", ".env"),
                    arguments.get("template_path", ".env.example"),
                )
            case "dotenv_diff":
                return self._diff(arguments["path_a"], arguments["path_b"])
            case _:
    ...
    def _read(self, path: str) -> str:
        ...
        if not env:
            return "Empty or no valid entries."
    ...
    def _diff(self, path_a: str, path_b: str) -> str:
        ...
        for k in all_keys:
            if k not in a:
                lines.append(f"  + {k} (only in {path_b})")
                diffs += 1
            elif k not in b:
                lines.append(f"  - {k} (only in {path_a})")
                diffs += 1
            elif a[k] != b[k]:
                lines.append(f"  ~ {k} (different values)")
                diffs += 1
        if diffs == 0:
            lines.append("  Files are identical.")
        else:
            lines.insert(1, f"  {diffs} difference(s)")
        return "\n".join(lines)
