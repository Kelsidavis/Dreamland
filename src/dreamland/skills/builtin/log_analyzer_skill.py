...
class LogAnalyzerSkill(Skill):
    ...
    def description(self) -> str:
        return "Parse, filter, and summarize log files"

    def tools(self) -> list[ToolDefinition]:
        return [
            ToolDefinition(
                name="log_summary",
                description="Summarize a log file — line count, error rate, time range, top errors",
                parameters={
                    "type": "object",
                    "properties": {
                        ...
                        "lines": {
                            "type": "integer",
                            "description": "Max lines to analyze (default: 10000)",
                        },
                    },
                    "required": ["path"],
                },
            ...
            ToolDefinition(
                name="log_filter",
                description="Filter log lines by level, pattern, or time range",
                parameters={
                    "type": "object",
                    "properties": {
                        "path": {"type": "string", "description": "Path to log file"},
                        "level": {
                            "type": "string",
                            "description": "Log level filter (ERROR, WARN, etc.)",
                        },
                        "pattern": {"type": "string", "description": "Regex pattern to match"},
                        "tail": {
                            "type": "integer",
                            "description": "Only last N lines (default: all)",
                        },
                    },
                    "required": ["path"],
                },
            ...
            ToolDefinition(
                name="log_errors",
                description="Extract and group error messages from a log file",
                parameters={
                    "type": "object",
                    "properties": {
                        "path": {"type": "string", "description": "Path to log file"},
                        "top": {
                            "type": "integer",
                            "description": "Number of top errors (default: 10)",
                        },
                    },
                    "required": ["path"],
                },
    ...
    async def execute(self, tool_name: str, arguments: dict[str, Any]) -> Any:
        match tool_name:
            case "log_summary":
                return self._summary(arguments["path"], arguments.get("lines", 10000))
            case "log_filter":
                return self._filter(
                    arguments["path"],
                    arguments.get("level"),
            ...
            case "log_errors":
                return self._errors(arguments["path"], arguments.get("top", 10))
            case _:
    ...
    def _filter(self, path: str, level: str | None, pattern: str | None, tail: int | None) -> str:
        ...
        for line in lines:
            if level:
                lm = _LOG_PATTERNS["level"].search(line)
                if not lm or lm.group(1).upper() != level.upper():
                    continue
            if pattern:
