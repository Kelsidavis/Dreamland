...
class MathSkill(Skill):
    ...
    def description(self) -> str:
        return "Statistics, number formatting, and mathematical functions"
    ...
    def tools(self) -> list[ToolDefinition]:
        return [
            ...
            ToolDefinition(
                name="math_format",
                description=(
                    "Format a number (comma separators, "
                    "scientific notation, percentages, bytes)"
                ),
                parameters={
                    "type": "object",
                    "properties": {
            ...
            ToolDefinition(
                name="math_sequence",
                description="Generate number sequences (range, fibonacci, primes)",
                parameters={
                    "type": "object",
                    "properties": {
                        "type": {
                            "type": "string",
                            "enum": ["range", "fibonacci", "primes"],
                            "description": "Sequence type",
                        },
                        "count": {
                            "type": "integer",
                            "description": "How many numbers (default: 10)",
                        },
    ...
    async def execute(self, tool_name: str, arguments: dict[str, Any]) -> Any:
        match tool_name:
            case "math_stats":
                return self._stats(arguments["numbers"])
            case "math_format":
                return self._format(arguments["number"], arguments["format"])
            case "math_sequence":
    ...
    def _to_roman(self, n: int) -> str:
        if n <= 0 or n > 3999:
            return f"{n} (out of range for Roman numerals)"
        vals = [
            (1000, "M"),
            (900, "CM"),
            (500, "D"),
