...
class PlaceholderSkill(Skill):
    ...
    def description(self) -> str:
        return "Generate test data — lorem ipsum, fake names, emails, sample JSON"
    ...
    def tools(self) -> list[ToolDefinition]:
        return [
            ...
            ToolDefinition(
                name="fake_users",
                description="Generate fake user data (name, email, age)",
                parameters={
                    "type": "object",
                    "properties": {
            ...
            ToolDefinition(
                name="fake_data",
                description="Generate random data of a specific type",
                parameters={
                    "type": "object",
                    "properties": {
                        "type": {
                            "type": "string",
                            "enum": ["email", "phone", "ip", "date", "color", "url", "sentence"],
    ...
    async def execute(self, tool_name: str, arguments: dict[str, Any]) -> Any:
        match tool_name:
            case "lorem":
                return self._lorem(arguments.get("words", 50), arguments.get("paragraphs", 1))
            case "fake_users":
                return self._users(arguments.get("count", 5), arguments.get("format", "json"))
            case "fake_data":
                return self._data(arguments["type"], arguments.get("count", 5))
            case _:
    ...
    def _users(self, count: int, fmt: str) -> str:
        ...
        if fmt == "csv":
            ...
            for u in users:
                lines.append(f"{u['name']},{u['email']},{u['age']}")
            return "\n".join(lines)
        elif fmt == "table":
            lines = [f"{'Name':<20} {'Email':<35} {'Age':>3}"]
            lines.append("-" * 60)
            for u in users:
                lines.append(f"{u['name']:<20} {u['email']:<35} {u['age']:>3}")
            return "\n".join(lines)
        ...
        for _ in range(min(count, 100)):
            match dtype:
                case "email":
                    ...
                    items.append(f"{local}@{domain}")
                case "phone":
                    a = random.randint(200, 999)
                    b = random.randint(100, 999)
                    c = random.randint(1000, 9999)
                    items.append(f"+1-{a}-{b}-{c}")
                case "ip":
                    parts = [
                ...
                case "date":
                    y = random.randint(20, 29)
                    mo = random.randint(1, 12)
                    d = random.randint(1, 28)
                    items.append(f"20{y}-{mo:02d}-{d:02d}")
                case "color":
