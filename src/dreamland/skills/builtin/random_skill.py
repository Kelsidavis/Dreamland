...
class RandomSkill(Skill):
    ...
    def description(self) -> str:
        return "Random generators — dice, coins, choices, shuffles"

    def tools(self) -> list[ToolDefinition]:
        return [
            ToolDefinition(
                name="roll_dice",
                description="Roll dice (e.g., 2d6, 1d20, 3d8+5)",
                parameters={
                    "type": "object",
                    "properties": {
                        "notation": {
                            "type": "string",
                            "description": "Dice notation (NdS or NdS+M)",
                        },
                    },
                    "required": ["notation"],
                },
            ...
            ToolDefinition(
                ...
                parameters={
                    ...
                    "properties": {
                        ...
                        "count": {
                            "type": "integer",
                            "description": "How many to pick (default: 1)",
                        },
                    },
                    "required": ["options"],
                },
    ...
    async def execute(self, tool_name: str, arguments: dict[str, Any]) -> Any:
        match tool_name:
            case "roll_dice":
                return self._dice(arguments["notation"])
            case "flip_coin":
                return self._coin(arguments.get("count", 1))
            case "random_choice":
                return self._choice(arguments["options"], arguments.get("count", 1))
            case "random_number":
                return self._number(
                    arguments.get("min", 1), arguments.get("max", 100), arguments.get("count", 1)
                )
            case _:
    ...
    def _dice(self, notation: str) -> str:
        ...
        if not m:
            return f"Invalid notation: {notation} (use NdS or NdS+M)"
