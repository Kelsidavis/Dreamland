...
class DateFormatSkill(Skill):
    ...
    def description(self) -> str:
        return "Parse and convert dates between formats"

    def tools(self) -> list[ToolDefinition]:
        return [
            ToolDefinition(
                name="date_convert",
                description="Convert a date string between formats",
                parameters={
                    "type": "object",
                    "properties": {
                        ...
                        "to_format": {
                            "type": "string",
                            "description": f"Target: {', '.join(_FORMATS.keys())}",
                        },
                    },
    ...
    async def execute(self, tool_name: str, arguments: dict[str, Any]) -> Any:
        if tool_name == "date_now":
            ...
            for name, fmt in _FORMATS.items():
                if fmt == "unix":
                    lines.append(f"  {name}: {int(now.timestamp())}")
                elif fmt == "relative":
                    continue
                else:
                    lines.append(f"  {name}: {now.strftime(fmt)}")
            return "\n".join(lines)
        elif tool_name == "date_convert":
            dt = self._parse(arguments["date"])
            if not dt:
                return f"Cannot parse: {arguments['date']}"
            fmt = _FORMATS.get(arguments["to_format"])
            if not fmt:
                return f"Unknown format: {arguments['to_format']}"
            if fmt == "unix":
