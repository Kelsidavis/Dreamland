...
def _run_codex_exec(prompt: str, model: str) -> str:
    fd, output_path = tempfile.mkstemp(prefix="dreamland-codex-compact-", suffix=".txt")
    os.close(fd)
...
def codex_compact_text(
    ...
    selected_model = str(model or os.environ.get("DREAMLAND_CODEX_MODEL") or _DEFAULT_MODEL)
...
class CodexSkill(Skill):
    ...
    async def _compact(self, args: dict[str, Any]) -> str:
        ...
        model = str(args.get("model") or os.environ.get("DREAMLAND_CODEX_MODEL") or _DEFAULT_MODEL)
