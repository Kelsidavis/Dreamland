...
class NpmSkill(Skill):
    ...
    def description(self) -> str:
        return "Inspect package.json — dependencies, scripts, and project info"

    def tools(self) -> list[ToolDefinition]:
        return [
            ToolDefinition(
                name="npm_info",
                description="Show package.json summary (name, version, deps, scripts)",
                parameters={
                    "type": "object",
                    "properties": {
                        "path": {
                            "type": "string",
                            "description": "Path to package.json or project dir (default: cwd)",
                        },
                    },
                },
            ),
            ToolDefinition(
                ...
                parameters={
                    ...
                    "properties": {
                        "path": {
                            "type": "string",
                            "description": "Path to package.json or project dir",
                        },
                        "dev": {
                            "type": "boolean",
            ...
            ToolDefinition(
                name="npm_audit_check",
                description=(
                    "Check for known issues in dependencies "
                    "(reads package.json only, no network)"
                ),
                parameters={
                    "type": "object",
                    "properties": {
    ...
    async def execute(self, tool_name: str, arguments: dict[str, Any]) -> Any:
        match tool_name:
            case "npm_info":
                return self._info(arguments.get("path", "."))
            case "npm_deps":
                return self._deps(arguments.get("path", "."), arguments.get("dev", True))
            case "npm_scripts":
                return self._scripts(arguments.get("path", "."))
            case "npm_audit_check":
                return self._audit(arguments.get("path", "."))
            case _:
    ...
    def _info(self, path: str) -> str:
        ...
        if pkg.get("engines"):
            lines.append(f"  Engines: {json.dumps(pkg['engines'])}")
        return "\n".join(lines)
    ...
    def _audit(self, path: str) -> str:
        ...
        for name, ver in all_deps.items():
            if ver in ("*", "latest"):
                issues.append(f"  [!] {name}: {ver} (unpinned)")
            elif ver.startswith("git") or ver.startswith("http"):
                issues.append(f"  [!] {name}: {ver} (URL dependency)")
            elif "file:" in ver:
                issues.append(f"  [!] {name}: {ver} (local file)")
        if not issues:
            return f"No issues found in {len(all_deps)} dependencies."
