...
class ManSkill(Skill):
    ...
    def description(self) -> str:
        return "Look up command man pages and --help output"

    def tools(self) -> list[ToolDefinition]:
        return [
            ToolDefinition(
                name="man_page",
                description="Show the man page for a command (summary section)",
                parameters={
                    "type": "object",
                    "properties": {
                        "command": {
                            "type": "string",
                            "description": "Command name (e.g., grep, curl, git)",
                        },
                        "section": {
                            "type": "string",
                            "description": "Man section (1-8, default: auto)",
                        },
                    },
                    "required": ["command"],
                },
            ...
            ToolDefinition(
                name="man_help",
                description="Run --help for a command and return output",
                parameters={
                    "type": "object",
                    "properties": {
            ...
            ToolDefinition(
                name="man_tldr",
                description="Show a concise cheat-sheet style summary of a command",
                parameters={
                    "type": "object",
                    "properties": {
    ...
    async def execute(self, tool_name: str, arguments: dict[str, Any]) -> Any:
        match tool_name:
            case "man_page":
                return await self._man(arguments["command"], arguments.get("section"))
            case "man_help":
                return await self._help(arguments["command"])
            case "man_tldr":
                return await self._tldr(arguments["command"])
            case _:
