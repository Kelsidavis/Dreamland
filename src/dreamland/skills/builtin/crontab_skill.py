...
class CrontabSkill(Skill):
    ...
    def description(self) -> str:
        return "Read and manage the system crontab"

    def tools(self) -> list[ToolDefinition]:
        return [
            ToolDefinition(
                name="crontab_list",
                description="Show the current user's crontab entries",
                parameters={"type": "object", "properties": {}},
            ),
            ToolDefinition(
                name="crontab_add",
                description="Add a new crontab entry",
                parameters={
                    "type": "object",
                    "properties": {
                        "schedule": {
            ...
            ToolDefinition(
                name="crontab_remove",
                description="Remove a crontab entry by line number",
                parameters={
                    "type": "object",
                    "properties": {
                        "line": {
                            "type": "integer",
                            "description": "Line number (from crontab_list, 1-based)",
                        },
                    },
                    "required": ["line"],
                },
        ...
        match tool_name:
            ...
            case "crontab_add":
                return await self._add(
                    arguments["schedule"], arguments["command"], arguments.get("comment")
                )
            case "crontab_remove":
                return await self._remove(arguments["line"])
            case _:
    ...
    async def _list(self) -> str:
        ...
        return (
            f"Crontab ({len(entries)} lines):\n" + "\n".join(entries)
            if entries
            else "Crontab is empty."
        )
    ...
    async def _remove(self, line: int) -> str:
        ...
        if line < 1 or line > len(lines):
            return f"Invalid line: {line} (crontab has {len(lines)} lines)"
