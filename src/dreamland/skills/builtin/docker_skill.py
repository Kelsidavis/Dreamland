...
class DockerSkill(Skill):
    ...
    def description(self) -> str:
        return "Manage Docker containers, images, and services"

    def tools(self) -> list[ToolDefinition]:
        return [
            ToolDefinition(
                name="docker_ps",
                description="List running containers (like docker ps)",
                parameters={
                    "type": "object",
                    "properties": {
                        "all": {
    ...
    async def execute(self, tool_name: str, arguments: dict[str, Any]) -> Any:
        match tool_name:
            ...
            case "docker_logs":
                return await self._run_docker(
                    ["logs", "--tail", str(arguments.get("tail", 50)), arguments["container"]]
                )
            case "docker_inspect":
                raw = await self._run_docker(
                    [
                        "inspect",
                        "--format",
                        "Name: {{.Name}}\nImage: {{.Config.Image}}\nStatus: {{.State.Status}}\n"
