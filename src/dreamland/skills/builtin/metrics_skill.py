...
class MetricsSkill(Skill):
    ...
    def description(self) -> str:
        return "Track custom counters, gauges, and timers for monitoring"
    ...
    def tools(self) -> list[ToolDefinition]:
        return [
            ...
            ToolDefinition(
                name="metric_report",
                description="Show all tracked metrics",
                parameters={
                    "type": "object",
                    "properties": {
                        "name": {
            ...
            ToolDefinition(
                name="metric_reset",
                description="Reset all or specific metrics",
                parameters={
                    "type": "object",
                    "properties": {
                        "name": {
                            "type": "string",
                            "description": "Metric to reset (or omit for all)",
                        },
                    },
                },
            ),
    ...
    async def execute(self, tool_name: str, arguments: dict[str, Any]) -> Any:
        match tool_name:
            ...
            case "metric_timer":
                ...
                return (
                    f"Timer {name}: {arguments['duration_ms']}ms "
                    f"(avg: {avg:.1f}ms, count: {len(vals)})"
                )
            case "metric_report":
                return self._report(arguments.get("name"))
            case "metric_reset":
    ...
    def _report(self, name: str | None) -> str:
        ...
        for k, vals in sorted(_timers.items()):
            ...
            lines.append(
                f"  timer/{k}: avg={avg:.1f}ms min={mn:.1f}ms max={mx:.1f}ms count={len(vals)}"
            )
