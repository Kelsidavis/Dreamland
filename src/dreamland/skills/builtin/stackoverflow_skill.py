...
class StackOverflowSkill(Skill):
    ...
    def description(self) -> str:
        return "Search Stack Overflow questions and answers"

    def tools(self) -> list[ToolDefinition]:
        return [
            ToolDefinition(
                name="so_search",
                description="Search Stack Overflow for questions",
                parameters={
                    "type": "object",
                    "properties": {
    ...
    async def execute(self, tool_name: str, arguments: dict[str, Any]) -> Any:
        ...
        params: dict[str, Any] = {
            "order": "desc",
            "sort": "relevance",
            "intitle": arguments["query"],
            "site": "stackoverflow",
            "pagesize": min(arguments.get("limit", 5), 10),
            "filter": "!nNPvSNdWme",
        }
        if arguments.get("tagged"):
            params["tagged"] = arguments["tagged"]
        try:
