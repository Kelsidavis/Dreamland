...
class SemverSkill(Skill):
    ...
    def description(self) -> str:
        return "Parse, compare, and bump semantic versions"

    def tools(self) -> list[ToolDefinition]:
        return [
            ToolDefinition(
                name="semver_parse",
                description="Parse a semantic version string into components",
                parameters={
                    "type": "object",
                    "properties": {
                        "version": {
                            "type": "string",
                            "description": "Version string (e.g., 1.2.3, v2.0.0-beta)",
                        },
                    },
                    "required": ["version"],
                },
            ...
            ToolDefinition(
                name="semver_bump",
                description="Bump a version (major, minor, or patch)",
                parameters={
                    "type": "object",
                    "properties": {
                        ...
                        "prerelease": {
                            "type": "string",
                            "description": "Prerelease tag (e.g., beta, rc.1)",
                        },
                    },
            ...
            ToolDefinition(
                name="semver_compare",
                description="Compare two versions (which is newer?)",
                parameters={
                    "type": "object",
                    "properties": {
    ...
    async def execute(self, tool_name: str, arguments: dict[str, Any]) -> Any:
        match tool_name:
            case "semver_parse":
                return self._parse(arguments["version"])
            case "semver_bump":
                return self._bump(
                    arguments["version"], arguments["bump"], arguments.get("prerelease")
                )
            case "semver_compare":
                return self._compare(arguments["a"], arguments["b"])
            case _:
    ...
    def _compare(self, a: str, b: str) -> str:
        ...
        tb = (pb[0], pb[1], pb[2])
        if ta > tb:
            return f"{a} is NEWER than {b}"
        if ta < tb:
            return f"{a} is OLDER than {b}"
