...
class WebhookTriggerSkill(Skill):
    ...
    def description(self) -> str:
        return "Send outbound webhooks to Slack, Discord, or any URL"

    def tools(self) -> list[ToolDefinition]:
        return [
            ToolDefinition(
                name="webhook_send",
                description="Send a POST to a webhook URL with JSON payload",
                parameters={
                    "type": "object",
                    "properties": {
    ...
    async def execute(self, tool_name: str, arguments: dict[str, Any]) -> Any:
        ...
        from dreamland.netguard import check_url
        ...
        match tool_name:
            case "webhook_send":
                return await self._send(
                    arguments["url"], arguments["payload"], arguments.get("headers")
                )
            case "slack_message":
                return await self._slack(
            ...
            case "discord_message":
                return await self._discord(
                    arguments["webhook_url"], arguments["content"], arguments.get("username")
                )
            case _:
    ...
    async def _send(self, url: str, payload: dict, headers: dict | None) -> str:
        ...
        h = {"Content-Type": "application/json", "User-Agent": "Dreamland/1.0"}
        if headers:
            h.update(headers)
        try:
