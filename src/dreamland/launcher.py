"""Dreamland launcher — a tiny HTTP daemon that spawns ``dreamland worker`` on demand.

The launcher lets a coordinator (or any client with the shared bearer token)
ask a remote host to start a fresh Dreamland worker process without SSH access.
A host running ``dreamland launcher`` listens on an HTTP port and exposes:

  - ``GET /health``        — liveness probe (no auth)
  - ``POST /launch``       — spawn a ``dreamland worker`` subprocess (token required)
...
per-pid file under ``$DREAMLAND_HOME/launcher-logs/`` and waits ~1s after
...
Auth: a shared bearer token sourced from the ``DREAMLAND_TRIGGER_TOKEN`` env
...
log = logging.getLogger("dreamland.launcher")

DEFAULT_PORT = 18751
TOKEN_ENV = "DREAMLAND_TRIGGER_TOKEN"
...
LAUNCHER_LOG_DIR = DREAMLAND_HOME / "launcher-logs"
...
def _build_worker_argv(payload: dict[str, Any]) -> tuple[list[str], str | None]:
    """Build the argv for ``dreamland worker`` from a launch payload.
    ...
    # Find the dreamland binary — fall back to ``python -m dreamland.cli.main`` style
    # invocation if the binary isn't on PATH (e.g. an editable install in a
    # virtualenv that wasn't activated for this process).
    binary = shutil.which("dreamland")
    if binary:
        argv = [binary, "worker"]
    else:
        argv = ["python", "-m", "dreamland", "worker"]
...
def _spawn_worker(
    ...
    individual variables — useful for forwarding ``DREAMLAND_HOME`` or specifying
...
_DEFAULT_UPGRADE_COMMANDS: dict[str, list[str]] = {
    ...
    # dreamland is installed as a regular package.
    "pip": ["pip", "install", "--upgrade", "dreamland"],
    ...
    "uv": ["uv", "pip", "install", "--upgrade", "dreamland"],
}


# Public alias — re-used by `dreamland worker --auto-update` so the in-process
...
def self_upgrade_and_reexec(strategy: str) -> bool:
    ...
    * ``dreamland worker --auto-update`` runs this before connecting so a
    ...
    * The coordinator can send a ``self_upgrade`` WS message to a running
    ...
    code. ``DREAMLAND_AUTO_UPDATE_DONE=1`` is set on the re-exec env so the
    ...
    env = dict(os.environ)
    env["DREAMLAND_AUTO_UPDATE_DONE"] = "1"
    # sys.argv[0] is whatever invoked us (bare "dreamland" on PATH, or the
...
def build_app(token: str) -> Starlette:
    ...
    async def health(_request: Request) -> JSONResponse:
        return JSONResponse({"status": "ok", "service": "dreamland-launcher"})
    ...
    async def launch(request: Request) -> JSONResponse:
        ...
        try:
            ...
            return JSONResponse(
                {"error": f"dreamland binary not found: {exc}"}, status_code=500
            )
...
def run(host: str = "0.0.0.0", port: int = DEFAULT_PORT) -> None:
    """Run the launcher daemon. Reads the bearer token from ``DREAMLAND_TRIGGER_TOKEN``."""
    import uvicorn
    ...
    # with the other Dreamland daemons.
    from dreamland.logging_setup import configure_terminal_logging

    configure_terminal_logging()
    log.info("Dreamland launcher listening on http://%s:%d (token from $%s)", host, port, TOKEN_ENV)
