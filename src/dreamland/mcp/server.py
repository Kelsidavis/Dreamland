"""MCP server façade for the Dreamland memory store.
...
``mcp`` Python SDK so Dreamland keeps zero new runtime deps for this
...
from dreamland.memory.store import MEMORY_TYPES, MemoryStore, salience

log = logging.getLogger("dreamland.mcp")


MCP_PROTOCOL_VERSION = "2024-11-05"
SERVER_NAME = "dreamland-memory"
# Match dreamland's package version so MCP clients can pin or warn.
try:
        ...
        try:
            if method == "initialize":
                ...
                result = {
                    ...
                    "serverInfo": {"name": SERVER_NAME, "version": _dreamland_version},
                }
...
def serve_stdio(store: MemoryStore | None = None) -> None:
    ...
    log.info(
        "Dreamland MCP server starting on stdio (protocol=%s, version=%s)",
        MCP_PROTOCOL_VERSION, _dreamland_version,
    )
    for line in sys.stdin:
        ...
        try:
            ...
            sys.stderr.write(f"dreamland mcp: bad JSON: {exc}\n")
            continue
