"""mDNS service advertisement and discovery for Dreamland.

The coordinator advertises ``_dreamland._tcp.local.`` so workers on the LAN
...
log = logging.getLogger("dreamland.gateway.mdns")

SERVICE_TYPE = "_dreamland._tcp.local."
...
def _local_ip() -> str:
    ...
    in DreamlandConfig for the operator override.
    """
    try:
...
class DreamlandServiceAdvertiser:
    ...
    async def start(self) -> None:
        ...
        self._info = AsyncServiceInfo(
            SERVICE_TYPE,
            f"dreamland-controller-{self.hostname}.{SERVICE_TYPE}",
...
async def discover_controller(timeout: float = DEFAULT_TIMEOUT_S) -> DiscoveredController | None:
    """Browse mDNS for a Dreamland coordinator.  Returns the first one found."""
    state = _BrowseState()
