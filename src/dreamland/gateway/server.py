"""Gateway server — the central nervous system of Dreamland.
...
from dreamland.agent.tool_parser import parse_tool_calls
from dreamland.config import DREAMLAND_HOME, DreamlandConfig
from dreamland.gateway.context_sync import ContextSyncManager
from dreamland.gateway.dispatcher import REASON_NO_WORKERS, Dispatcher
from dreamland.gateway.handoff import HandoffManager, HandoffReason
from dreamland.gateway.idle_tasks import IDLE_TASK_PROMPTS, IdleTaskManager
...
log = logging.getLogger("dreamland.gateway")
...
class GatewayServer:
    ...
    workspace_root: Path = field(
        default_factory=lambda: DREAMLAND_HOME / "workspaces"
    )
    ...
    async def start(self) -> None:
        ...
        try:
            from dreamland.gateway.mdns import DreamlandServiceAdvertiser

            self._mdns_advertiser = DreamlandServiceAdvertiser(
                port=gw.port,
                ...
                if msg_type == "message":
                    ...
                    try:
                        ...
                        if stream:
                            ...
                            # and OpenAI-compat dreamland.quality_degraded.
    ...
    async def _verify_pass(
        ...
        from dreamland.agent.conversation import Role as _Role
        ...
        from dreamland.gateway.sessions import Session as _Session
    ...
    async def _ensemble_dispatch(
        ...
        from dreamland.nodes.roles import NodeRole as _NodeRole
        now = _dt.now(UTC)
    ...
    def _pick_alternate_chat_worker(
        ...
        # it.
        from dreamland.nodes.capability import _safe_int
    ...
    async def _route_by_role(
        ...
        *, task_type_override: str | None = None,
        ...
        to :class:`dreamland.gateway.dispatcher.Dispatcher` for the actual worker
    ...
    async def dispatch_role_task(
        ...
        *,
        ...
        Implements the :class:`dreamland.agent.orchestrator.RoleDispatcher`
        ...
        if (
            ...
            from dreamland.agent.conversation import Message as _Message
    ...
    async def _run_git(
        ...
        proc = await asyncio.create_subprocess_exec(
            "git", "-C", workspace,
            "-c", "user.email=dreamland@local",
            "-c", "user.name=Dreamland Orchestrator",
            *args,
    ...
    async def _dispatch_idle_task(self, worker: WorkerInfo) -> None:
        ...
        from dreamland.agent.project import load_project_context
    ...
    async def _schedule_idle_work(self) -> None:
        ...
        # See DreamlandConfig.idle_tasks_enabled.
        if not getattr(self.config, "idle_tasks_enabled", False):
            return
        for worker in self._workers.list():
            if (
    ...
    def _build_http_app(self) -> Starlette:
        ...
        async def memory_inspect(request: Request) -> JSONResponse:
            ...
            try:
                from dreamland.memory.store import salience as _salience
        ...
        async def fleet_spawn(request: Request) -> JSONResponse:
            ...
            Operators run ``dreamland launcher`` on each candidate worker host;
        ...
        async def memory_create(request: Request) -> JSONResponse:
            ...
            """
            from dreamland.memory.store import MEMORY_TYPES
        ...
        async def webchat(_request: Any) -> HTMLResponse | FileResponse:
            ...
            return HTMLResponse("<h1>Dreamland</h1><p>Web UI not found.</p>", status_code=404)
        ...
        async def conversation_export(request: Request) -> HTMLResponse:
            ...
            from dreamland.persistence.export import (
                export_html,
                export_json,
                export_markdown,
                export_text,
            ...
            #
            # Prefer the conversation's title when set — "dreamland-How-
            # to-deploy.md" is more meaningful than the typical
            # session_id ("dreamland-openai-chatcmp.md"). Falls back to
            ...
            filename = f"dreamland-{stem[:32]}.{ext}"
            return Response(
                content,
        ...
        async def simple_ask(request: Request) -> JSONResponse:
            ...
            try:
                ...
                # whether dreamland even tried hard. Distinct from
        ...
        async def api_orchestrate(request: Request) -> JSONResponse:
            ...
            if not background:
                ...
                if git_enabled:
                    ...
                    await self._git_commit_workspace(
                        workspace_dir,
                        f"{achieved}: {goal[:60]} [dreamland:{oid}]",
                    )
                self._persist_orchestration(
        ...
        async def api_orchestrate_archive(request: Request):
            ...
            return _Response(
                ...
                headers={
                    "Content-Disposition":
                        f'attachment; filename="dreamland-{oid}.zip"',
                },
            )
        ...
        from dreamland.gateway.openai_compat import build_openai_routes  # noqa: F401
        ...
        from dreamland.setup_server import setup_routes
