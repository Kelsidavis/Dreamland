"""Remote worker client for controller-managed Dreamland execution."""
...
log = logging.getLogger("dreamland.gateway.worker")
...
def default_worker_capabilities(
    ...
    # Advertise the worker's `dreamland` version so the coordinator can
    ...
    try:
        from dreamland import __version__ as _dreamland_version
    except Exception:
        _dreamland_version = "0.0.0"
    ...
    caps: dict[str, Any] = {
        ...
        "tools": allow_tools,
        "dreamland_version": _dreamland_version,
    }
    ...
    # away.
    try:
        from dreamland.launcher import get_last_upgrade_attempt
...
def _detect_local_launcher() -> str | None:
    """Detect a co-located ``dreamland launcher`` daemon on this host.

    Strategy:
    1. If ``$DREAMLAND_LAUNCHER_URL`` is set, trust it verbatim — operator
    ...
    """
    override = os.environ.get("DREAMLAND_LAUNCHER_URL", "").strip()
    if override:
        return override

    from dreamland.launcher import DEFAULT_PORT
...
def _detect_available_models(
    ...
    *,
    ...
    skip launching ``dreamland worker --model X`` against a host that would
