...
Makes Dreamland a drop-in replacement for any OpenAI client.
...
``system_fingerprint`` (per-process stable, derived from the dreamland
package version), and the usual ``usage`` block.

Dreamland-specific extensions (OpenAI's spec allows extra body fields,
...
Usage with the openai Python SDK:
    ...
    client = OpenAI(base_url="http://127.0.0.1:18743/v1", api_key="dreamland")
...
def _system_fingerprint() -> str:
    ...
    harnesses). Derive from the dreamland package version so the
    fingerprint flips on coordinator upgrades, matching OpenAI's
    behaviour of changing fingerprints on model revisions.
    """
    try:
        from dreamland import __version__ as _v
    except Exception:
        _v = "0"
    import hashlib as _hash
    return "fp_dreamland_" + _hash.sha256(_v.encode("utf-8")).hexdigest()[:10]
...
def build_openai_routes(
    ...
    *,
    ...
    async def chat_completions(request: Request) -> JSONResponse | StreamingResponse:
        ...
        `stream=true`), and surfaces Dreamland-specific collaboration
        state under a `dreamland` namespaced field on non-streaming
        responses.

        See the module docstring for the full param schema,
        including dreamland extensions (`verify`, `ensemble`).
        """
        try:
        ...
        # collaboration modes here so non-dreamland clients (LangChain,
        ...
        # Dreamland uses its own tool system (skills loaded server-side)
        ...
        # an explicit ``tools_ignored`` flag in the dreamland-namespaced
        ...
        if client_tools is not None:
            if not isinstance(client_tools, list) or not client_tools:
                ...
                _tools_log.getLogger("dreamland.openai_compat").warning(
                    "Request supplied %d tool definition(s); dreamland "
                    ...
                    "dreamland.tools_ignored response field.",
                    len(client_tools),
                )
        ...
        # — those clients deserve to know dreamland doesn't support
            ...
            if stream:
                ...
                if gateway is not None and messages:
                    ...
                    try:
                        ...
                        import logging
                        logging.getLogger("dreamland.openai_compat").debug(
                    ...
                    if ensemble_raw:
                        ...
                        if getattr(gateway, "_dispatcher", None) is not None:
                            try:
                                ...
                                pass
                        if arbitrated:
                            from dreamland.agent.conversation import Message
                            response = Message(
                        ...
                        if worker is not None:
                            if intent == "chat":
                                ...
                                try:
                                    ...
                                    _v1_log.getLogger("dreamland.openai_compat").info(
                ...
                if completion_tokens == 0 and response.content:
                    from dreamland.agent.context import count_tokens_fallback
                ...
                try:
                    ...
                    # clients ignore it and Dreamland-aware clients can
                    # see whether verify/ensemble actually ran. Sits
                    # under a single `dreamland` key rather than at the
                    # top level so we never collide with a future
                    # OpenAI field. Only emitted when there's
                    # something to report.
                    dreamland_meta: dict[str, Any] = {}
                    if meta.get("verified_by"):
                        dreamland_meta["verified_by"] = meta["verified_by"]
                        dreamland_meta["verifier_corrected"] = bool(
                            meta.get("verifier_corrected", False)
                        )
                        dreamland_meta["primary_worker"] = meta.get(
                        ...
                        if meta.get("primary_original_answer") is not None:
                            dreamland_meta["primary_original_answer"] = meta[
                        ...
                        # signal in the dreamland-namespaced block.
                        dreamland_meta["verify_skipped"] = True
                        dreamland_meta["verify_skip_reason"] = (
                    ...
                    if meta.get("ensemble"):
                        dreamland_meta["ensemble"] = True
                        if meta.get("ensemble_arbitration"):
                            dreamland_meta["ensemble_arbitration"] = (
                        ...
                        dreamland_meta["ensemble_skipped"] = True
                        if not _contribs:
                            dreamland_meta["ensemble_skip_reason"] = (
                        ...
                        dreamland_meta["ensemble_contributions"] = _contribs
                    if meta.get("fallback_from_worker"):
                        dreamland_meta["fallback_from_worker"] = (
                            meta["fallback_from_worker"]
                        )
                        dreamland_meta["fallback_reason"] = meta.get(
                            "fallback_reason", ""
                        )
                    if meta.get("dual_empty_text"):
                        dreamland_meta["dual_empty_text"] = True
                        if meta.get("alt_worker"):
                            dreamland_meta["alt_worker"] = meta["alt_worker"]
                    if tools_ignored:
                        ...
                        # see the same signal.
                        dreamland_meta["tools_ignored"] = True
                    ...
                    if gateway is not None and openai_session_id is not None:
                        ...
                        if dispatcher is not None:
                            ...
                            if last_dec is not None and last_dec.quality_degraded:
                                dreamland_meta["quality_degraded"] = True
                    if dreamland_meta:
                        completion["dreamland"] = dreamland_meta
    ...
    *,
    ...
    try:
        ...
        if not yielded_any and fallback_agent is not None and fallback_conv is not None:
            import logging
            logging.getLogger("dreamland.openai_compat").warning(
...
def _format_completion(
    ...
    *,
    ...
    Dreamland-specific metadata (``verified_by``, ``ensemble``,
    ``verify_skipped``, ``dual_empty_text``, …) is attached by the
    chat_completions handler under a top-level ``dreamland`` key, NOT
    ...
    return {
        ...
        # dreamland version flips it on coordinator upgrades, which is
