...
- ``POST /api/setup/save``  →  validate, write ``~/.dreamland/config.toml``

It can run two ways:

1. **Standalone**: ``dreamland setup`` boots a one-shot Starlette server, opens the
   browser, and the server keeps running until the user closes the tab or hits
   Ctrl-C. This is what first-time users get — no need to have ``dreamland serve``
   already running.
...
log = logging.getLogger("dreamland.setup")
...
async def _probe_backends(config: DreamlandConfig) -> dict[str, dict[str, Any]]:
...
def _apply_form_to_config(config: DreamlandConfig, form: dict[str, Any]) -> tuple[bool, str | None]:
    ...
    return JSONResponse(
        {
            ...
            "config_path": str(DREAMLAND_HOME / "config.toml"),
        }
    )
...
async def _save_handler(request: Request) -> JSONResponse:
    ...
    log.info("Setup wizard wrote %s (backend=%s)", DREAMLAND_HOME / "config.toml", config.backend)
    return JSONResponse(
        {
            "ok": True,
            "config_path": str(DREAMLAND_HOME / "config.toml"),
...
def build_app() -> Starlette:
    """Build the standalone setup app served by ``dreamland setup``."""
    routes = setup_routes()
...
def run_standalone(host: str = "127.0.0.1", port: int = 18749, open_browser: bool = True) -> None:
    ...
    # confirmation that the wizard wrote ~/.dreamland/config.toml).
