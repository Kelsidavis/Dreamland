"""Discord bot channel — run Dreamland as a Discord bot.

Connects to the Dreamland gateway via WebSocket and bridges messages
between Discord and the agent.

Setup:
  1. Create a Discord bot at https://discord.com/developers
  2. Get the bot token
  3. Run: dreamland discord --token YOUR_BOT_TOKEN

The bot responds to:
  - Direct messages
  - Messages mentioning the bot
  - Messages starting with a configurable prefix (default: !dreamland)
"""
...
from dreamland.channels.base import Channel

log = logging.getLogger("dreamland.channels.discord")
...
class DiscordChannel(Channel):
    ...
    async def _handle_dispatch(self, event: str, data: dict) -> None:
        ...
        # Get response from Dreamland gateway
        try:
