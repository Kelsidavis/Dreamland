"""Telegram bot channel — run Dreamland as a Telegram bot.
...
Setup:
  ...
  2. Get the bot token
  3. Run: dreamland telegram --token YOUR_BOT_TOKEN
"""
...
log = logging.getLogger("dreamland.channels.telegram")
...
class TelegramChannel(Channel):
    ...
    async def _handle_update(self, update: dict) -> None:
        ...
        if text == "/start":
            await self._send(
                chat_id,
                "Hey! I'm Dreamland. It doesn't exist. Just send me a message.",
            )
            return
