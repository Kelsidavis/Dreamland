"""CLI command for Dreamland health status and diagnostics."""
...
from dreamland.cli import cli
from dreamland.persistence.conversation import ConversationStore
...
def get_python_process_stats() -> dict[str, Any]:
    """Get stats for the current Python process (and Dreamland agent if found)."""
    try:
        import psutil
    except ImportError:
        return {}
...
def calculate_health_score(stats: dict[str, Any]) -> int:
    ...
    # Dreamland process health
    dreamland_proc = stats.get("dreamland_process", {})
    if dreamland_proc and dreamland_proc.get("memory", {}).get("percent"):
        proc_mem = dreamland_proc["memory"]["percent"]
...
def status(as_json: bool, verbose: bool, history: bool) -> None:
    """Display Dreamland health status and diagnostics."""
    console = Console()
    ...
    stats = {
        ...
        "dreamland_process": get_python_process_stats(),
    ...
    if history:
        history_path = Path("~/.dreamland/status_history.json").expanduser()
        history_data: list = []
        if history_path.exists():
            ...
            # still run ``dreamland status --history`` without first
    ...
    if as_json:
        ...
        panel = Panel(
            ...
            title=(
                f"[bold]🧼 Dreamland v0.7.3[/bold] • [bold]{health_label}[/bold]"
                f" ([bold]{health_score}/100[/bold])"
            ),
...
def build_status_tree(stats: dict[str, Any], verbose: bool = False) -> Tree:
    ...
    tree = Tree("🧼 Dreamland Status")

    # System Overview
    system = tree.add("⚙️ System Overview")
    sys_info = stats.get("system", {})
    system.add(f"OS: Darwin {sys_info.get('timestamp', '')[:10]}")
    system.add(f"Load: {' '.join(str(v) for v in sys_info.get('load_avg', (0, 0, 0)))}")

    mem = sys_info.get("memory", {})
    memory_text = (
        f"{mem.get('used_gb', 0):.1f}/{mem.get('total_gb', 0):.1f} GB ({mem.get('percent', 0)}%)"
    )
    mem_style = "green" if mem.get("percent", 0) < 80 else "orange"
    system.add(Text(f"Memory: {memory_text}", style=mem_style))

    if verbose:
        swap = mem.get("swap", {})
        if swap:
            system.add(
                f"Swap: {swap.get('used_gb', 0):.1f}/{swap.get('total_gb', 0):.1f} GB"
                f" ({swap.get('percent', 0)}%)"
            )

    # Dreamland Runtime
    dreamland = tree.add("⚙️ Dreamland Runtime")
    dreamland_proc = stats.get("dreamland_process", {})
    mem = dreamland_proc.get("memory", {})

    dreamland.add(
        Text(
            f"Process: PID {dreamland_proc.get('pid', 'N/A')} "
            f"({dreamland_proc.get('name', 'python')})",
            style="cyan",
        )
    )
    dreamland.add(
    ...
    dreamland.add(f"Context: {model.get('context_window', 8192)} tokens")
    ...
    session.add(
        Text(
            f"Messages: {session.get('messages', 0)}"
            f" ({session.get('avg_messages_per_conv', 0):.1f}/conv)",
            style="green",
        )
    )
    session.add(
    ...
    if verbose:
        ...
        detailed.add(f"CPU: {dreamland_proc.get('cpu_percent', 0)}% → contrib: ~20")
        detailed.add(f"Dreamland Process Memory: {mem.get('percent', 0)}% → contrib: ~20")
