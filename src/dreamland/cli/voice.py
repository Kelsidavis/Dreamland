"""Voice input — speak to Dreamland using your microphone.

Uses mlx-whisper for on-device speech-to-text on Apple Silicon.
No audio leaves your machine.

Usage:
    dreamland voice              listen and transcribe once
    dreamland voice --chat       continuous voice chat mode
    dreamland voice --file a.wav transcribe an audio file
"""
...
log = logging.getLogger("dreamland.cli.voice")
console = Console()
...
def check_voice_deps() -> str | None:
    ...
    try:
        ...
        return "mlx-whisper not installed. Run: pip install dreamland[voice]"
...
def record_audio(duration: float = 10.0, sample_rate: int = 16000) -> bytes | str:
    ...
    try:
        ...
        return "sounddevice not installed. Run: pip install dreamland[voice]"
