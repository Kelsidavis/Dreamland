...
Commands:
  ...
  /context             Show loaded project context (.dreamland.md)
...
HELP_TEXT = """[bold]Chat commands:[/bold]
  ...
  [green]/context[/green]             Show loaded .dreamland.md project context
...
def _cmd_info(ctx: SlashContext) -> None:
    from dreamland.agent.project import find_project_contexts
    ...
    if contexts:
        ...
        console.print("  [green]Project context:[/green] [dim]none (create .dreamland.md)[/dim]")
...
def _cmd_stats(ctx: SlashContext) -> None:
    ...
    console.print(
        ...
        f"{len(asst_msgs)} dreamland, {len(tool_msgs)} tool"
    )
    ...
    if total_tokens > 0:
        ...
        console.print("    [green]Dreamland (local): $0.00[/green]")
...
def _cmd_fork(ctx: SlashContext, arg: str) -> None:
    ...
    from dreamland.agent.conversation import Conversation, Message
    ...
    console.print(f"[dim]Resume original later: dreamland resume {old_id}[/dim]")
...
def _cmd_skills(ctx: SlashContext) -> None:
    """In-chat shortcut for ``dreamland skills`` — list loaded skills + tools.
...
def _cmd_newagent(ctx: SlashContext, arg: str) -> None:
    ...
    name = parts[0]
    model = parts[1]
    identity = parts[2] if len(parts) > 2 else f"You are Dreamland ({name} mode). It doesn't exist."
...
def _cmd_context(ctx: SlashContext) -> None:
    from dreamland.agent.project import find_project_contexts, load_project_context
    ...
    if not paths:
        ...
        console.print()
        console.print("  [green]echo '# My Project' > .dreamland.md[/green]")
        return
...
def _cmd_export(ctx: SlashContext, arg: str) -> None:
    from dreamland.persistence.export import export_html, export_markdown
...
def _cmd_snippet(ctx: SlashContext, arg: str) -> None:
    from dreamland.cli.snippets import remove_snippet, set_snippet
...
def _cmd_unalias(ctx: SlashContext, arg: str) -> None:
    from dreamland.cli.aliases import remove_alias
...
def _cmd_health(ctx: SlashContext) -> None:
    ...
    if not hb:
        ...
        console.print("[dim]Heartbeat starts automatically with `dreamland serve`.[/dim]")
        return
...
def _cmd_delegate(ctx: SlashContext, arg: str) -> bool | None:
    ...
    from dreamland.agent.orchestrator import ROLE_PROMPTS
