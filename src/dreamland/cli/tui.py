"""TUI — Terminal User Interface for Dreamland.
...
def build_skills_panel(config: DreamlandConfig) -> Panel:
...
def build_system_panel(config: DreamlandConfig) -> Panel:
...
def build_channels_panel() -> Panel:
    channels = [
        ("CLI", "dreamland chat", "green"),
        ("Web", "dreamland serve", "green"),
        ("Discord", "dreamland discord -t TOKEN", "blue"),
        ("Telegram", "dreamland telegram -t TOKEN", "blue"),
        ("Slack", "dreamland slack -b/-a TOKEN", "blue"),
        ("Webhook", "dreamland webhook", "cyan"),
    ]
    table = Table(show_header=False, box=None, padding=(0, 1))
...
def build_commands_panel() -> Panel:
    ...
    for cmd, desc in cmds:
        table.add_row(f"[green]dreamland {cmd}[/green]", f"[dim]{desc}[/dim]")
    return Panel(table, title="Key Commands", border_style="white")


def render_tui(config: DreamlandConfig | None = None) -> Layout:
