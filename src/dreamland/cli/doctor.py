"""Dreamland doctor — diagnose your setup and find problems before they find you.
...
log = logging.getLogger("dreamland.cli.doctor")
console = Console()
...
def run_doctor(config: DreamlandConfig | None = None) -> list[Check]:
    """Run all diagnostic checks. Returns list of Check results."""
    config = config or DreamlandConfig.load()
    checks: list[Check] = []
...
def check_environment() -> Check:
    ...
    # Disk space for ~/.dreamland
    try:
        usage = shutil.disk_usage(DREAMLAND_HOME.parent)
...
def check_config(config: DreamlandConfig) -> Check:
    ...
    if config_path.exists():
        ...
        c.suggestions.append("Run: dreamland setup  (browser GUI)  or  dreamland init")
...
def check_gpu(config: DreamlandConfig) -> Check:
    """Detect local GPUs and report VRAM + a rough model-size capacity.

    Dreamland never offloads to the GPU itself — llama-server / Ollama / MLX do —
    ...
    c = Check("GPU")
    try:
        from dreamland.agent.discovery import detect_gpus
...
def check_mlx(config: DreamlandConfig | None = None) -> Check:
...
def _check_llama_model(c: Check, config: DreamlandConfig) -> Check:
    ...
    try:
        from dreamland.agent.discovery import scan_gguf_models
    ...
    if not models:
        ...
        c.suggestions.append(
            "Start your llama-server (e.g. ~/.dreamland/launch-llama.sh), or place "
            "a .gguf in ~/models or ~/.dreamland/models"
        )
        return c
...
def check_model(config: DreamlandConfig) -> Check:
...
def check_skills(config: DreamlandConfig) -> Check:
    """Check skill loading."""
    c = Check("Skills")
...
def _probe_missing_persisted_workers(c: Check, host: str, http_port: int) -> None:
    ...
    ``$DREAMLAND_HOME/worker_state.json``. When a previously-registered
...
def _probe_fleet_endpoints(c: Check, host: str, http_port: int) -> None:
    ...
    try:
        ...
        if workers:
            ...
            # capability advertisement now carries `dreamland_version` —
            # flag any worker that doesn't match the coordinator.
            try:
                from dreamland import __version__ as _coord_version
            except Exception:
                _coord_version = "0.0.0"
            mismatched: list[str] = []
            unknown: list[str] = []
            for w in workers:
                wv = (w.get("capabilities") or {}).get("dreamland_version")
                if not wv:
            ...
            if unknown:
                ...
                # signal.
                c.warn(
                    f"{len(unknown)} worker(s) don't advertise dreamland_version "
...
def check_gateway(config: DreamlandConfig) -> Check:
    ...
    try:
        ...
        # red border, but operators running `dreamland doctor` from the
    ...
    c.ok("Gateway not running (start with: dreamland serve)")
...
def check_persisted_worker_state() -> Check:
    ...
    in ``$DREAMLAND_HOME/worker_state.json``. When operators wonder "why is this
...
def check_orchestrations() -> Check:
    ...
    if failed:
        c.warn(f"{failed} run(s) failed — inspect via `dreamland orchestrations`")

    ws_root = DREAMLAND_HOME / "workspaces"
    if ws_root.is_dir():
        total = 0
        count = 0
...
def check_memory_embeddings() -> Check:
    ...
    """
    from dreamland.memory import embeddings
    ...
    if embeddings.is_available():
        ...
        c.suggestions.append(
            ...
            "pip install 'dreamland[embeddings]'"
        )

    # Dimension consistency across stored vectors. Mixed dims usually
    # mean $DREAMLAND_EMBED_MODEL changed without a re-encode pass; the
    ...
    if len(dims) > 1:
        ...
        c.suggestions.append(
            "Re-encode every row: dreamland memory reembed --all"
        )
    return c
...
def check_memory_store() -> Check:
    ...
    """
    from dreamland.memory.store import DEFAULT_MEMORY_DIR, MemoryStore
    ...
    try:
        ...
        if count > 5 and log_size == 0:
            ...
            c.suggestions.append(
                ...
                "`dreamland memory search QUERY` to populate the log."
            )
        elif log_size > 0:
            ...
            if log_size >= cap:
                c.warn(
                    ...
                    "pruned. Bump DreamlandConfig.memory_recall_log_cap "
    ...
    if entries:
        ...
        if len(unused_auto) >= 10 and len(unused_auto) >= len(entries) // 2:
            ...
            c.suggestions.append(
                "Run `dreamland memory stats` to inspect, then `dreamland memory "
        ...
        if cold:
            ...
            c.suggestions.append(
                "Review the regex in src/dreamland/memory/auto_capture.py; "
...
def check_storage() -> Check:
    ...
    # Check DREAMLAND_HOME
    if DREAMLAND_HOME.exists():
        c.ok(f"Dreamland home: {DREAMLAND_HOME}")
    else:
        c.ok(f"Dreamland home not initialized ({DREAMLAND_HOME})")
        c.suggestions.append("Run: dreamland setup  (browser GUI)  or  dreamland init")
