"""Single source of truth for ``basicConfig`` in Dreamland's long-running CLI entry points.

Each of ``dreamland serve`` / ``dreamland worker`` / ``dreamland setup`` / ``dreamland launcher``
...
def configure_terminal_logging(level: int = logging.INFO) -> None:
    """Configure ``logging.basicConfig`` for a long-running Dreamland command.
