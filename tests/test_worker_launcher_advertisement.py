"""Workers advertise the URL of a co-located ``dreamland launcher`` daemon so
...
from dreamland.gateway.worker_client import _detect_local_launcher
...
class TestLauncherDetection:
    def test_no_launcher_returns_none(self):
        ...
        with patch.dict("os.environ", {}, clear=False), \
            ...
            # Make sure DREAMLAND_LAUNCHER_URL isn't set in the live env.
            import os

            old = os.environ.pop("DREAMLAND_LAUNCHER_URL", None)
            try:
                assert _detect_local_launcher() is None
            finally:
                if old is not None:
                    os.environ["DREAMLAND_LAUNCHER_URL"] = old

    def test_env_override_trusted_verbatim(self):
        with patch.dict(
            "os.environ",
            {"DREAMLAND_LAUNCHER_URL": "http://special-host:9999"},
        ):
